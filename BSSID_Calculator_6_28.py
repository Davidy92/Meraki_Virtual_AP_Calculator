...
def writer(name, physical_mac, entry_24, entry_5):
    ...
    '''
    #Add a space after the 16th MAC
    if (SPACING_COUNTER % 16) == 0 and SPACING_COUNTER != 0:
        worksheet.write(SPACING_COUNTER, 5 , "")
        worksheet.write(SPACING_COUNTER, 4, "")
        EXCEL_COUNTER += 1
    ...
    #print virtual ssids in columns C = 2.4, D = 5
    worksheet.write(SSID_FORMAT, 3, "SSID " + str(SSID_COUNTER))
    worksheet.write(SSID_FORMAT, 4, entry_24)
    worksheet.write(SSID_FORMAT, 5, entry_5)

    SPACING_COUNTER += 1
    EXCEL_COUNTER += 1
    SSID_FORMAT += 1

    if (SSID_FORMAT % 15) == 0 and SSID_FORMAT != 0:
        worksheet.write(SSID_FORMAT, 3, "MESH NETWORK MAC")
        SSID_COUNTER = 0
        SSID_FORMAT += 1
        '''
